...
// Solid enrichment: finds /contact, /about, /contact-us pages and extracts from them
class Enricher {
    ...
    $('a').each((_,a)=>{
      ...
      if(href.includes('contact') || text.includes('contact') || text.includes('get in touch') || href.includes('about')){
        try{
          ...
          if(full.startsWith(baseUrl) || full.includes(new URL(baseUrl).hostname)) links.push(full);
        }catch{}
      }
    });
  ...
  async enrichFromContactPages(baseUrl, baseHtml, campaign){
    ...
    try{
      ...
      allWaLinks.push(...wa.links);
      allWaNums.push(...wa.numbers);
      social={...extractSocial(baseHtml)};

      // Crawl contact pages for more solid data
      ...
      for(const cp of contactPages){
        try{
          ...
          allWaLinks.push(...wa2.links);
          allWaNums.push(...wa2.numbers);
          const s=extractSocial(res.data);
          social={...social, ...s};
        }catch{}
      }
    }catch(e){}
    ...
    return {
      companyName: company,
      emails: [...new Set(allEmails)].slice(0,10),
      phones: [...new Set(allPhones)].slice(0,5),
      whatsapp: { links:[...new Set(allWaLinks)], numbers:[...new Set(allWaNums)] },
      social
    };
  }
}
