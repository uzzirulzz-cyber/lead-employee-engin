...
async function runCampaign(jobId){
  ...
  console.log(`[runner v2] ${jobId} stage=${job.stage} ${campaign.progress}/${campaign.targetCount}`);
  ...
  try{
    if(job.stage==='DISCOVERY'){
      ...
      for(const d of discovered){
        await prisma.discoverySource.create({ data:{ campaignId:campaign.id, source:d.source, sourceUrl:d.url, query:d.query }});
      }
    ...
    if(['FETCHING','ENRICHING','VALIDATING'].includes(job.stage)){
      ...
      await Promise.all(sources.map(src=> limit(async ()=>{
        try{
          const res=await axios.get(src.sourceUrl,{ headers:{ 'User-Agent':'LeadForgeBot/2.0 (+https://leadforge.app)' }, timeout:12000 });
          const enriched=await enricher.enrichFromContactPages(src.sourceUrl, res.data, campaign);

          // Create leads for each email found, plus fallback if only phone/WhatsApp
          ...
          for(const email of emailsToProcess){
            ...
            if(exists){
              await prisma.lead.update({ where:{ id:exists.id }, data:{ lastSeen:new Date(), sourceCount:{ increment:1 } }});
              batchDup++;
              continue;
            }
            ...
            if(email){
              ...
              emailStatus=v.status; confidence=v.confidence;
            }
            const leadData={
              companyName: enriched.companyName,
              industry: campaign.industry,
              category: campaign.industry,
              website: src.sourceUrl,
              domain,
              email,
              emailStatus,
              emailConfidence: confidence,
              phoneRaw: enriched.phones[0]||null,
              phoneNormalized: enriched.phones[0]||null,
              whatsappRaw: enriched.whatsapp.numbers[0]||null,
              whatsappNormalized: enriched.whatsapp.numbers[0]||null,
              whatsappDetected: enriched.whatsapp.links.length>0 || enriched.whatsapp.numbers.length>0,
              whatsappUrl: enriched.whatsapp.links[0]||null,
              whatsappSourceUrl: src.sourceUrl,
              country: campaign.country,
              city: campaign.city,
              linkedinUrl: enriched.social.linkedin,
              facebookUrl: enriched.social.facebook,
              instagramUrl: enriched.social.instagram,
              source: src.source,
              sourceUrl: src.sourceUrl,
              sourceCount:1,
              fingerprint:fp
            };
            leadData.leadScore=require('../lib/extractors').scoreLead(leadData);
            await prisma.lead.create({ data:{ ...leadData, campaignId:campaign.id }});
            batchValid++;
          }
          await prisma.discoverySource.update({ where:{ id:src.id }, data:{ processed:true, enriched:true }});
        }catch(e){
          await prisma.discoverySource.update({ where:{ id:src.id }, data:{ processed:true }});
        }
      })));
      ...
      await prisma.campaign.update({ where:{ id:campaign.id }, data:{
        progress:total,
        extracted:{ increment: batchValid },
        valid:{ increment: batchValid },
        duplicates:{ increment: batchDup },
        enriched:{ increment: batchValid },
        currentStage:`ENRICHING batch ${job.batchIndex+1} - ${total}/${campaign.targetCount}`
      }});
      await prisma.job.update({ where:{ id:jobId }, data:{ batchIndex:{ increment:1 } }});

      if(total < campaign.targetCount){
        setTimeout(()=>runCampaign(jobId), 800);
      } else {
        await prisma.campaign.update({ where:{ id:campaign.id }, data:{ status:'COMPLETED', currentStage:'COMPLETED' }});
        await prisma.job.update({ where:{ id:jobId }, data:{ status:'COMPLETED' }});
      }
    }
  }catch(err){
    console.error(err);
    await prisma.job.update({ where:{ id:jobId }, data:{ status:'FAILED', error:String(err) }});
