...
async function enrollLeads({ sequenceId, leadIds }){
  ...
  for(const leadId of leadIds){
    ...
    const nextSendAt = new Date(); // immediate for step 0
    // If first step has delay, add it
    if(firstStep.delayDays || firstStep.delayHours){
...
async function processDueSequences(){
  console.log('[sequences] checking due enrollments');
  const now = new Date();
  const due = await prisma.sequenceEnrollment.findMany({ 
    where:{ status:'ACTIVE', nextSendAt:{ lte: now } }, 
    include:{ sequence:{ include:{ steps:{ orderBy:{ stepIndex:'asc' }} }}, lead:true },
    take: 50
  });
  ...
  for(const enrollment of due){
    ...
    if(!canSend.allowed){
      console.log(`Warmup limit reached ${canSend.sent}/${canSend.limit}, skipping`);
      continue;
    }
    ...
    try{
      if(step.channel==='EMAIL'){
        ...
        await prisma.outreachLog.create({ data:{
          leadId: lead.id, campaignId: sequence.campaignId, sequenceId: sequence.id,
          channel:'EMAIL', stepIndex: step.stepIndex, subject, message: body, status:'SENT', sentAt: new Date()
        }});
        await incrementWarmup();
      } else if(step.channel==='WHATSAPP'){
        // Log only, user clicks manually - but we still advance sequence
        const message = renderTemplate(step.body||'Hi {{companyName}}', lead);
        await prisma.outreachLog.create({ data:{
          leadId: lead.id, campaignId: sequence.campaignId, sequenceId: sequence.id,
          channel:'WHATSAPP', stepIndex: step.stepIndex, message, status:'PENDING'
        }});
      }
      ...
      await prisma.outreachLog.create({ data:{
        leadId: lead.id, campaignId: sequence.campaignId, sequenceId: sequence.id,
        channel: step.channel, stepIndex: step.stepIndex, status:'FAILED', error: String(e.message)
      }});
      // Retry later - push nextSendAt by 2 hours
